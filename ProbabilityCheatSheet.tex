...
g\vspace{-3em}
	...
	\begin{tabular}{|c|c|c|}
		\hline
		$F_{X \vert A}(x) = \frac{F_X(x) - F_X(a)}{F_X(b) - F_X(a)} \text{  if } a \leq x < b$ & $f_{X \vert A}(x) = \frac{f_X(x)}{P(A)} \text{  if A is some event between } a \leq X < b$  & $f_{X \vert Y}(x, y) = \frac{f_{XY}(x, y)}{f_Y(y)}$\\
		\hline
